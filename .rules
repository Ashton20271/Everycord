...
- Every NEW source file starts with SPDX header:
   ...
   * Copyright (c) <year> OpenCord contributors
   * SPDX-License-Identifier: GPL-3.0-or-later
   */
  ```
  Header says `OpenCord, a Discord client mod` on new OpenCord files. Keep older upstream GPL preambles and attribution intact when editing existing upstream files.
  Older files carry the full GPL preamble. Leave existing headers alone, don't rewrite them.
- Folder per plugin. `src/plugins/<name>/index.tsx`, `src/equicordplugins/<name>/index.tsx`, or `src/everycordplugins/<name>/index.tsx`. Single-file plugins still get a folder.
...
- `isModified?: true` marks an upstream Vencord plugin OpenCord modified.
...
- Never reference build flags literally inside `replace`. They only exist in OpenCord's bundle. Interpolate outside:
  ```js
  // BAD
...
- Renderer side:
    ```ts
    const Native = VencordNative.pluginHelpers.MyPlugin as PluginNative<
...
| `IS_VESKTOP` | Vesktop |
| `IS_EQUIBOP` | Equibop |
| `VERSION` | OpenCord version string |
