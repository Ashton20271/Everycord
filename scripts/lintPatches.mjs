...
const tracked = execFileSync("git", ["ls-files", "src"], { cwd: ROOT, encoding: "utf8" })
    .split("\n")
    .filter(p => /^src\/(plugins|equicordplugins|everycordplugins|illegalcordplugins|testcordplugins|esharqplugins|equicordplusplugins|mallcordplugins)\/.*\.(ts|tsx)$/.test(p))
